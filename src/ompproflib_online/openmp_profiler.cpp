	...
	ompt_task_id_t task_id)
{
	...
	#if DEBUG
	std::cout << "implicit task end popping barrier finish. node_id= " << last_nodes[tid].back()->incrId << std::endl;
	#endif 
	popNode(tid);
...
void OpenmpProfiler::GenerateProfile(){
	...
	TreeNode* root = last_nodes[0].back();
