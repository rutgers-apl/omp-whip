...
kmp_task_t *__kmp_task_alloc(ident_t *loc_ref, kmp_int32 gtid,
  ...
  KA_TRACE(10, ("__kmp_task_alloc(enter): T#%d loc=%p, flags=(0x%x) "
                "sizeof_task=%ld sizeof_shared=%ld entry=%p location=%s\n",
                gtid, loc_ref, *((kmp_int32 *)flags), sizeof_kmp_task_t,
                sizeof_shareds, task_entry, loc_ref->psource));
...
/*#if OMPT_SUPPORT
    if (ompt_enabled && ompt_callbacks.ompt_callback(ompt_event_task_alloc)) {
      ompt_callbacks.ompt_callback(ompt_event_task_alloc)();
    }
#endif*/
