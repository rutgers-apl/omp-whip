...
#define GUIDED_ANALYTICAL_WORKAROUND (x)
  ...
  if (ompt_enabled && ompt_callbacks.ompt_callback(ompt_event_loop_begin)) {
    ...
    ompt_callbacks.ompt_callback(ompt_event_loop_begin)(
        team_info->parallel_id, task_info->task_id, team_info->microtask, loc->psource);//nader
  }
#endif
}
...
int __kmpc_dispatch_next_4(ident_t *loc, kmp_int32 gtid, kmp_int32 *p_last,
  ...
  int n_dispatch_status = __kmp_dispatch_next<kmp_int32>(loc, gtid, p_last, p_lb, p_ub, p_st);
  #if OMPT_SUPPORT && OMPT_TRACE
  if (ompt_enabled && ompt_callbacks.ompt_callback(ompt_event_dispatch_next)) {
    
    ompt_callbacks.ompt_callback(ompt_event_dispatch_next)(
        *p_lb, *p_ub, *p_st, n_dispatch_status);//nader
  }
  #endif
  return n_dispatch_status;
  //start with this nader
}
...
int __kmpc_dispatch_next_4u(ident_t *loc, kmp_int32 gtid, kmp_int32 *p_last,
  ...
  int n_dispatch_status = __kmp_dispatch_next<kmp_uint32>(loc, gtid, p_last, p_lb, p_ub, p_st);
  #if OMPT_SUPPORT && OMPT_TRACE
  if (ompt_enabled && ompt_callbacks.ompt_callback(ompt_event_dispatch_next_others)) {
    
    ompt_callbacks.ompt_callback(ompt_event_dispatch_next_others)(n_dispatch_status);//nader
  }
  #endif
  return n_dispatch_status;
}
...
int __kmpc_dispatch_next_8(ident_t *loc, kmp_int32 gtid, kmp_int32 *p_last,
  ...
  int n_dispatch_status = __kmp_dispatch_next<kmp_int64>(loc, gtid, p_last, p_lb, p_ub, p_st);
  #if OMPT_SUPPORT && OMPT_TRACE
  if (ompt_enabled && ompt_callbacks.ompt_callback(ompt_event_dispatch_next_others)) {
...
int __kmpc_dispatch_next_8u(ident_t *loc, kmp_int32 gtid, kmp_int32 *p_last,
  ...
  int n_dispatch_status = __kmp_dispatch_next<kmp_uint64>(loc, gtid, p_last, p_lb, p_ub, p_st);
  #if OMPT_SUPPORT && OMPT_TRACE
  if (ompt_enabled && ompt_callbacks.ompt_callback(ompt_event_dispatch_next_others)) {
