...
int __kmp_fork_call(ident_t *loc, int gtid,
                    ...
                    void *unwrapped_task,
#endif
                    microtask_t microtask, launch_t invoker,//nader
...
KA_TRACE(20, ("OMP_parallel_begin_nader: T#%d loc = %s\n", gtid, loc->psource));
#if OMPT_SUPPORT
    if (ompt_enabled &&
      ...
      ompt_callbacks.ompt_callback(ompt_event_parallel_begin)(
          ompt_task_id, ompt_frame, ompt_parallel_id, team_size, unwrapped_task, loc->psource,
        ...
        if (ompt_enabled) {
          ...
          if (ompt_callbacks.ompt_callback(ompt_event_implicit_task_begin)) {
            ompt_callbacks.ompt_callback(ompt_event_implicit_task_begin)(
                my_parallel_id, my_task_id, loc->psource);
          }
#endif
...
// Get args from parent team for teams construct
          ...
          if (ompt_enabled) {
            ...
            if (ompt_callbacks.ompt_callback(ompt_event_implicit_task_begin)) {
              ompt_callbacks.ompt_callback(ompt_event_implicit_task_begin)(
                  ompt_parallel_id, my_task_id, loc->psource);
            }
#endif
...
//int __kmp_invoke_task_func(int gtid, ident_t *loc) {
...
// (they hang on fork barrier until next parallel)
  ...
  __kmp_fork_call(loc, gtid, fork_context_intel, team->t.t_argc,
                  ...
                  VOLATILE_CAST(launch_t) __kmp_invoke_task_func, NULL);//nader
#if INCLUDE_SSC_MARKS
  SSC_MARK_JOINING();
#endif
