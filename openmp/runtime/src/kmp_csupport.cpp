...
void __kmpc_fork_call(ident_t *loc, kmp_int32 argc, kmpc_micro microtask, ...) {
  ...
  // maybe to save thr_state is enough here
  {
    ...
    __kmp_fork_call(loc, gtid, fork_context_intel, argc,
                    ...
                    VOLATILE_CAST(launch_t) __kmp_invoke_task_func,//nader
...
void __kmpc_fork_teams(ident_t *loc, kmp_int32 argc, kmpc_micro microtask,
  ...
  __kmp_fork_call(loc, gtid, fork_context_intel, argc,
                  ...
                  VOLATILE_CAST(launch_t) __kmp_invoke_teams_master,//nader
#if (KMP_ARCH_X86_64 || KMP_ARCH_ARM || KMP_ARCH_AARCH64) && KMP_OS_LINUX
                  &ap
#else
                  ap
...
int __kmpc_invoke_task_func(int gtid) { return __kmp_invoke_task_func(gtid); }//nader
...
void __kmpc_critical_with_hint(ident_t *loc, kmp_int32 global_tid,
  ...
  #if OMPT_SUPPORT //&& OMPT_BLAME nader
  if (ompt_enabled &&
      ompt_callbacks.ompt_callback(ompt_event_wait_critical)) {
        KA_TRACE(15, ("__kmpc_critical_with_hint_before_acquire T#%d\n", global_tid));
    ompt_callbacks.ompt_callback(ompt_event_wait_critical)((uint64_t)lck, loc->psource);
  }
  #endif
  ...
  if (ompt_enabled &&
      ompt_callbacks.ompt_callback(ompt_event_wait_critical)) {
        KA_TRACE(15, ("__kmpc_critical_with_hint_before_acquire else T#%d\n", global_tid));
    ompt_callbacks.ompt_callback(ompt_event_wait_critical)((uint64_t)lck, loc->psource);
  }
  #endif
  ...
  if (ompt_enabled &&
      ompt_callbacks.ompt_callback(ompt_event_acquired_critical)) {
        KA_TRACE(15, ("__kmpc_critical_with_hint_acquired T#%d\n", global_tid));
    ompt_callbacks.ompt_callback(ompt_event_acquired_critical)((uint64_t)lck, loc->psource);
  }
  #endif

  KA_TRACE(15, ("__kmpc_critical w hint: done T#%d\n", global_tid));
...
void __kmpc_end_critical(ident_t *loc, kmp_int32 global_tid,
  ...
  //KA_TRACE(15, ("__kmpc_end_nader start criticial T#%d KMP_USE_DYNAMIC_LOCK=%d\n", global_tid,KMP_USE_DYNAMIC_LOCK));
...
#if OMPT_SUPPORT //&& OMPT_BLAME nader
  KA_TRACE(15, ("__kmpc_end_nader without dynamic lock !!!should not happen: done T#%d\n", global_tid));
  if (ompt_enabled &&
      ompt_callbacks.ompt_callback(ompt_event_release_critical)) {
        KA_TRACE(15, ("__kmpc_end_Nader: done T#%d\n", global_tid));
  ...
  if (ompt_enabled &&
      ompt_callbacks.ompt_callback(ompt_event_release_critical)) {
        KA_TRACE(15, ("__kmpc_end_nader before callback: done T#%d\n", global_tid));
    ompt_callbacks.ompt_callback(ompt_event_release_critical)((uint64_t)lck, loc->psource);
  }
  #endif
...
kmp_int32 __kmpc_single(ident_t *loc, kmp_int32 global_tid) {
  ...
  if (ompt_enabled) {
    if (rc) {
      ...
      if (ompt_callbacks.ompt_callback(ompt_event_single_in_block_begin_wloc)) {
        ompt_callbacks.ompt_callback(ompt_event_single_in_block_begin_wloc)(
            team->t.ompt_team_info.parallel_id,
            team->t.t_implicit_task_taskdata[tid].ompt_task_info.task_id,
            team->t.ompt_team_info.microtask,
            loc->psource);
