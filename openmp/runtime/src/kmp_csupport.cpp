...
kmp_int32 __kmpc_master(ident_t *loc, kmp_int32 global_tid) {
  ...
  if (status) {
    if (ompt_enabled.ompt_callback_master) {
      ...
      ompt_callbacks.ompt_callback(ompt_callback_master)(//nader
          ompt_scope_begin, &(team->t.ompt_team_info.parallel_data),
          &(team->t.t_implicit_task_taskdata[tid].ompt_task_info.task_data),
          //OMPT_GET_RETURN_ADDRESS(0));
          OMPT_GET_RETURN_ADDRESS(0), loc->psource);
    }
  }
#endif
...
kmp_int32 __kmpc_single(ident_t *loc, kmp_int32 global_tid) {
  ...
  if (ompt_enabled.enabled) {
    if (rc) {
      if (ompt_enabled.ompt_callback_work) {//nader single begin
        ompt_callbacks.ompt_callback(ompt_callback_work)(
            ompt_work_single_executor, ompt_scope_begin,
            &(team->t.ompt_team_info.parallel_data),
            &(team->t.t_implicit_task_taskdata[tid].ompt_task_info.task_data),
            //1, OMPT_GET_RETURN_ADDRESS(0));
            1, OMPT_GET_RETURN_ADDRESS(0), loc->psource);
      }
    } else {
      if (ompt_enabled.ompt_callback_work) {//nader single other begin
        ompt_callbacks.ompt_callback(ompt_callback_work)(
            ...
            //1, OMPT_GET_RETURN_ADDRESS(0));
            1, OMPT_GET_RETURN_ADDRESS(0), loc->psource);
        ompt_callbacks.ompt_callback(ompt_callback_work)(//nader single other end
...
void __kmpc_end_single(ident_t *loc, kmp_int32 global_tid) {
  ...
  if (ompt_enabled.ompt_callback_work) {//nader single end
    ompt_callbacks.ompt_callback(ompt_callback_work)(
...
void __kmpc_for_static_fini(ident_t *loc, kmp_int32 global_tid) {
  ...
  if (ompt_enabled.ompt_callback_work) {
    ...
    ompt_callbacks.ompt_callback(ompt_callback_work)(//nader loop end
        ompt_work_type, ompt_scope_end, &(team_info->parallel_data),
        //&(task_info->task_data), 0, OMPT_GET_RETURN_ADDRESS(0));
        &(task_info->task_data), 0, OMPT_GET_RETURN_ADDRESS(0), loc->psource);
  }
#endif
