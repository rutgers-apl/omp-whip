...
// ON A ZERO-TRIP LOOP (lower=1, upper=0,stride=1) - JPH June 23, 2009.
...
#if OMPT_SUPPORT && OMPT_OPTIONAL
    if (ompt_enabled.ompt_callback_work) {//nader loop begin
      ompt_callbacks.ompt_callback(ompt_callback_work)(
          ompt_work_type, ompt_scope_begin, &(team_info->parallel_data),
          //&(task_info->task_data), 0, codeptr);
          &(task_info->task_data), 0, codeptr, loc->psource);
    }
#endif
    ...
    *pstride =
    ...
    if (ompt_enabled.ompt_callback_work) {//nader loop begin
      ompt_callbacks.ompt_callback(ompt_callback_work)(
          ...
          &(task_info->task_data), *pstride, codeptr, loc->psource);
    }
#endif
    return;
  }
  ...
  if (ompt_enabled.ompt_callback_work) {//nader loop begin?
    ompt_callbacks.ompt_callback(ompt_callback_work)(
        ...
        &(task_info->task_data), trip_count, codeptr, loc->psource);
  }
#endif
